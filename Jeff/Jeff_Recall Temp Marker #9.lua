-- REAPER Lua Script: Recall Saved Position 9 and Unselect Items
...
function recallCursorPosition9()
    -- Retrieve the saved position from the global state
    local position9 = reaper.GetExtState("PositionManager", "Position9")
    
    -- Check if the position was saved
    if position9 == "" then
        reaper.ShowMessageBox("Position 9 not found. Please save Position 9 first.", "Error", 0)
        return
    end
    ...
    local cursorPos = tonumber(position9)
    if cursorPos then
