-- REAPER Lua Script: Recall Saved Position 2 and Unselect Items
...
function recallCursorPosition2()
    -- Retrieve the saved position from the global state
    local position2 = reaper.GetExtState("PositionManager", "Position2")
    
    -- Check if the position was saved
    if position2 == "" then
        reaper.ShowMessageBox("Position 2 not found. Please save Position 2 first.", "Error", 0)
        return
    end
