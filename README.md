...
- No new Wwise audio objects, object creation/deletion, project saves, SoundBank generation or conversion-setting changes. Native import targets only verified existing source IDs and their existing original filenames/subfolders. Wwise may obtain required WAV access through its source-control provider; Relay sends no separate checkout or WWU checkout command.
- Exact project identity, unique sound name, source ID, parent, active source, original path and destination content are checked. Wrong projects, shared originals, missing files and channel-count changes are rejected. If Wwise cannot obtain write access, its native import fails and Relay pauses. The current original is hashed immediately before each replacement; if it changes during staging, replacement is rejected. Earlier edits are not remembered as conflicts between renders.
