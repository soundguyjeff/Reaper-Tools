...
# Python opens Unicode filenames on Windows; stock Lua's C fopen in Lupa does not.
# Compile the same bytes without executing them or changing the script filename.
check=lua.eval('function(source,name)local fn,err=load(source,name,"t");return fn~=nil,err end')
for entry in entries:
    ...
    ok,error=check(data.decode('utf-8-sig'),'@'+path.name);assert ok,(path.name,error)
