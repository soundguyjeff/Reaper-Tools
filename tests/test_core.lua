...
test('invalid matched GUID is rejected',function()local t={};for k,v in pairs(link)do t[k]=v end;t.source_id='name';raises(function()C.validate_link(t,source(),{source()},pid,link.project_path)end)end)
local sound={id=sid,name='fire',type='Sound'}
test('filename automatically matches an existing sound',function()assert(C.match_sound(link.render,{sound})==sound)end)
test('matching ignores ASCII letter case and WAV extension case',function()assert(C.match_sound('D:\\FIRE.WAV',{sound})==sound)end)
test('unknown filename never falls back to another sound',function()local found,why=C.match_sound('D:\\new.wav',{sound});assert(not found and why:find('No existing',1,true))end)
test('duplicate sound names are ambiguous across containers',function()local found,why=C.match_sound(link.render,{sound,{id=cid,name='Fire',type='Sound'}});assert(not found and why:find('More than one',1,true))end)
test('containers with the same name are not audio targets',function()assert(not C.match_sound(link.render,{{id=cid,name='fire',type='RandomSequenceContainer'}}))end)
test('partial names and version suffixes are never guessed',function()assert(not C.match_sound('D:\\fire_v2.wav',{sound}));assert(not C.match_sound('D:\\fire.wav.bak',{sound}))end)
test('punctuation is matched literally',function()local s={id=sid,type='Sound',name='fire.[01]'};assert(C.match_sound('D:\\fire.[01].wav',{s})==s)end)
test('only the final WAV extension is removed',function()assert(C.sound_name('D:\\fire.wav.wav')=='fire.wav')end)
test('competing render paths cannot overwrite the same sound in one batch',function()
 local a={link={sound_id=sid,original=link.original}};local b={link={sound_id=sid,original=link.original}}
 C.reject_collisions({a,b});assert(not a.link and not b.link and a.skip_reason and b.skip_reason)
end)
test('different sounds pointing at the same original are both rejected',function()
 local a={link={sound_id=sid,original=link.original}};local b={link={sound_id=cid,original=link.original:upper()}}
 C.reject_collisions({a,b});assert(not a.link and not b.link)
end)
test('independent matched outputs survive batch validation',function()
 local a={link={sound_id=sid,original=link.original}};local b={link={sound_id=cid,original='D:\\other.wav'}}
 C.reject_collisions({a,b});assert(a.link and b.link)
end)
test('unreadable old report entry becomes a baseline, never a replay',function()
 local d=C.detector();d:baseline({{path=link.render,ok=false}});d:observe({{path=link.render,ok=true,stamp='1'}},10);assert(not d:take(20))
 d:observe({{path=link.render,ok=true,stamp='2'}},30);assert(#d:take(32)==1)
end)
